...
const getAllBills = async () => {
    const [rows] = await pool.query('SELECT * FROM bill ORDER BY created_at DESC');
    return rows;
};

// Get bill by ID with items
const getBillById = async (id) => {
    const [billRows] = await pool.query('SELECT * FROM bill WHERE id = ?', [id]);
...
const getBillsByPaymentType = async (type) => {
    const [rows] = await pool.query('SELECT * FROM bill WHERE payment_type = ? ORDER BY created_at DESC', [type]);
    return rows;
};

// Get bills by user
const getBillsByUser = async (userId) => {
    const [rows] = await pool.query('SELECT * FROM bill WHERE user_id = ? ORDER BY created_at DESC', [userId]);
    return rows;
};

// Delete bill
const deleteBill = async (id) => {
    await pool.query('DELETE FROM bill WHERE id = ?', [id]);
};
    ...
    try {
        ...
        for (const item of oldItems) {
            if (item.category === 'product') {
                await productModel.updateStock(item.item_id, item.quantity); // Add back
            } else if (item.category === 'paddy') {
                await paddyModel.updateStock(item.item_id, item.quantity);
            } else if (item.category === 'fertilizer_pesticide') {
                await chemicalModel.updateStock(item.item_id, item.quantity);
            } else if (item.category === 'equipment') {
                await equipmentModel.updateStock(item.item_id, item.quantity);
            }
        }
        ...
        for (const item of items) {
            ...
            if (item.category === 'product') {
                await productModel.updateStock(item.item_id, -item.quantity); // Deduct
            } else if (item.category === 'paddy') {
                await paddyModel.updateStock(item.item_id, -item.quantity);
            } else if (item.category === 'fertilizer_pesticide') {
                await chemicalModel.updateStock(item.item_id, -item.quantity);
